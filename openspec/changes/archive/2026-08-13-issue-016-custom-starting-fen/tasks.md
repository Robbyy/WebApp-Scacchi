...
- [x] 6.1 Rendere obbligatoria la `startingFen` per creazione e aggiornamento delle posizioni `MIDDLEGAME`/`ENDGAME`: payload mancante o vuoto rifiutato con `400` e campo `startingFen`, senza fallback silenzioso a `START_FEN`; Aperture e varianti legacy mantengono il comportamento esistente. Decisione verificata con test backend per creazione, aggiornamento atomico, FEN mancante e FEN vuota.
- [x] 6.2 Completare il flusso frontend per aggiungere e modificare l'albero delle mosse di una posizione custom: dopo il salvataggio da `PositionEditor` l'utente viene portato a `/variants/:id/edit`, dove `VariantEditor` carica la FEN custom, consente di aggiungere/modificare rami e persiste `startingFen`, `tree` e `moves`. Verificato con test frontend mirati e suite completa.
- [x] 6.3 Adeguare la navigazione laterale e l'editor per gli studi `MIDDLEGAME`/`ENDGAME`: usare «Posizioni», rimuovere i badge Bianco/Nero e nascondere il campo «Lato da allenare», mantenendo `color` come dato tecnico backend derivato dalla FEN. Le Aperture conservano terminologia, badge e selettore esistenti. Verificato con test frontend mirati e suite completa.
