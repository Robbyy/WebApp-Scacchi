...
- **Posizioni manuali (R25, `issue-016-custom-starting-fen`)**: editor visuale per piazzamento/rimozione pezzi, lato al tratto, arrocco ed en-passant; generazione e normalizzazione della FEN, associazione allo studio, salvataggio anche senza mosse e validazione backend della posizione e dell'albero dalla FEN scelta. Implementazione committata in `d38a507` e pushata sul branch dedicato. R25 resta in verifica: sono aperti il comportamento senza `startingFen`, il flusso UI per creare/modificare l'albero delle mosse e la terminologia/navigazione fuori dalle Aperture (task 6.1–6.3).
